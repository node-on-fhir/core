...
Meteor.methods({
   ...
   * @returns {Array} Array of protocol lists
   */
  async 'checklist.protocols.list'() {
    try {
  ...
  /**
   * Clone a protocol as a new working list
   * @param {String} protocolId ID of the protocol to clone
   * @returns {String} ID of the new list
   */
  async 'checklist.protocols.clone'(protocolId) {
    check(protocolId, String);

    if (!this.userId) {
      throw new Meteor.Error('not-authorized', 'You must be logged in to use protocols');
    }
  ...
  /**
   * Create a new protocol from an existing list
   * @param {String} listId ID of the list to convert to protocol
   * @returns {Number} Update count
   */
  async 'checklist.protocols.create'(listId) {
    check(listId, String);
   ...
   * Called on server startup to ensure default protocols exist
   */
  async 'checklist.protocols.initializeSystemTemplates'() {
