...
// =============================================================================
// MAIN COMPONENT
// Core Honeycomb Criteria (not in packages)
const CORE_CRITERIA = [
  {
    id: '170.315(a)(5)',
    criterion: 'Demographics',
...
function ReferenceAppPage(props) {
  ...
  const [certificationCriteria, setCertificationCriteria] = useState(CERTIFICATION_CRITERIA);
  const [coreCriteria, setCoreCriteria] = useState(CORE_CRITERIA);
  
  // Handle updating criteria
  const handleUpdateCriterion = (criterionId, field, value, isCore = false) => {
    if (isCore) {
      setCoreCriteria(prev => prev.map(criterion => 
        criterion.id === criterionId 
          ? { ...criterion, [field]: value }
          : criterion
      ));
    } else {
      setCertificationCriteria(prev => prev.map(criterion => 
        criterion.id === criterionId 
          ? { ...criterion, [field]: value }
          : criterion
      ));
    ...
    >
      <Container maxWidth="xl" style={containerStyle}>
        ...
        <Box>
          ...
          {/* Core Honeycomb Criteria Table */}
          <Card sx={{ mt: 3 }}>
            <CardHeader 
              title={
                <Stack direction="row" spacing={2} alignItems="center">
                  <Typography variant="h5">Core Honeycomb Criteria</Typography>
                  <Chip 
                    label={`${CORE_CRITERIA.filter(c => c.isImplemented).length}/${CORE_CRITERIA.length} Implemented`}
                    color={CORE_CRITERIA.filter(c => c.isImplemented).length === CORE_CRITERIA.length ? "success" : "default"}
                    size="small"
                  />
                </Stack>
              }
              subheader="Criteria implemented in the core Node-on-FHIR framework"
            />
            <CardContent sx={{ p: 0 }}>
              <TableContainer>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      <TableCell width="15%">Criterion</TableCell>
                      <TableCell>Description</TableCell>
                      <TableCell width="20%">Package</TableCell>
                      <TableCell align="center" width="8%">Algo</TableCell>
                      <TableCell align="center" width="12%">Implemented</TableCell>
                      <TableCell align="center" width="8%">v3</TableCell>
                      <TableCell align="center" width="8%">Tests</TableCell>
                      <TableCell align="center" width="10%">Validated</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {coreCriteria.map((criterion) => (
                      <TableRow key={criterion.id}>
                        ...
                        <TableCell align="center">
                          <Checkbox
                            size="small"
                            checked={criterion.hasAlgorithms}
                            onChange={() => handleUpdateCriterion(criterion.id, 'hasAlgorithms', !criterion.hasAlgorithms, true)}
                          />
                        </TableCell>
                        <TableCell align="center">
                          <Checkbox
                            size="small"
                            checked={criterion.isImplemented}
                            onChange={() => handleUpdateCriterion(criterion.id, 'isImplemented', !criterion.isImplemented, true)}
                          />
                        </TableCell>
                        <TableCell align="center">
                          <Checkbox
                            size="small"
                            checked={criterion.isV3}
                            onChange={() => handleUpdateCriterion(criterion.id, 'isV3', !criterion.isV3, true)}
                          />
                        </TableCell>
                        <TableCell align="center">
                          <Checkbox
                            size="small"
                            checked={criterion.hasTests}
                            onChange={() => handleUpdateCriterion(criterion.id, 'hasTests', !criterion.hasTests, true)}
                          />
                        </TableCell>
                        <TableCell align="center">
                          <Checkbox
                            size="small"
                            checked={criterion.hasValidated}
                            onChange={() => handleUpdateCriterion(criterion.id, 'hasValidated', !criterion.hasValidated, true)}
                          />
                        </TableCell>
                      </TableRow>
                    ))}
