...
% conformance is certified externally by the ONC-approved Inferno tool.
...
Commons / Chronicle. It is generated from the committed
\texttt{package-lock.json} (\texttt{lockfileVersion~3}) --- the exact
dependency graph resolved for the certified build --- so it documents the
...
resolved} in the lockfile. Because Chronicle pins several transitive
...
records the shipped artifact, which is the object under certification.
...
records how those obligations interact with Chronicle's own distribution
license. The headline finding: the dependency tree is overwhelmingly
...
\section{Chronicle's Own License}

The Chronicle application code is licensed \textbf{AGPL-3.0} (see
\texttt{LICENSE.md}). This is an intentional network-copyleft posture: an
operator running a modified Chronicle as a network service must offer that
modified source to its users. AGPL-3.0 is the \emph{outbound} license of the
first-party application; it is not imposed by any dependency. The workflow
...
\item \textbf{MPL-2.0 (file-level copyleft).} MPL is per-file: only if a
  ...
  be offered. Chronicle consumes these packages unmodified, so the only
...
\item \textbf{Dual-licensed with a GPL option.} Two packages offer a choice
  that includes a GPL variant; Chronicle elects the permissive alternative
...
\item \textbf{No imported strong copyleft.} There is no GPL-2.0-only or
  ...
  system is Chronicle's own outbound license.
