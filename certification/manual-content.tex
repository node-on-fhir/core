...
\IfFileExists{inferno-reports/latest-g10-summary.pdf}{%
  \ifdefined\ScrollEdition
    % Scroll edition: report pages ship at their native letter size ---
    % raw-pages mode exempts them from the measured-height override (they
    % carry no scroll position marks), and fitpaper adopts their dimensions.
    \ScrollRawPagesOn
    \includepdf[pages=-, fitpaper=true]{inferno-reports/latest-g10-summary.pdf}%
    \clearpage % flush the final inserted page while raw-pages mode is active
    \ScrollRawPagesOff
  \else
    \includepdf[pages=-, scale=0.92, pagecommand={\thispagestyle{plain}}]{inferno-reports/latest-g10-summary.pdf}%
  \fi
}{%
  \begin{quotation}
