...
// The Daisey reference bundle was an Atmosphere server Asset read via
// Assets.getTextAsync(). NPM workflow packages have no Assets API, so the bundle
// is imported directly (Rspack inlines the JSON as a parsed object) and the three
// former Assets reads now use JSON.stringify(daiseyBundle) to preserve the
...
Meteor.methods({
  ...
  'referenceApp.loadDaiseyPatient': async function() {
    ...
    try {
      ...
      let bundleJson;
      try {
        bundleJson = JSON.stringify(daiseyBundle);
      } catch (assetError) {
  ...
  'referenceApp.getDaiseyBundleJson': async function() {
    ...
    try {
      const assetPath = 'data/Daisy/Daisey627_Jackelyn13_Koelpin146_958c63b0-4a7f-2ee7-ef6a-e04df5931b4c.json';
      const bundleJson = JSON.stringify(daiseyBundle);
