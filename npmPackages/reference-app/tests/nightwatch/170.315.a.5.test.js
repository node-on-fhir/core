// npmPackages/reference-app/tests/nightwatch/170.315.a.5.test.js

const testUtils = require('../../../../tests/nightwatch/honeycomb/enable_autopublish/shared-test-utils');

module.exports = {
  tags: ['base-ehr', 'onc-certification', '170.315.a.5'],

  'Base EHR - 170.315(a)(5) - Demographics': function (browser) {
    browser
    ...
    testUtils.navigateUrl(browser, '/patients/new');
    browser
    ...
    browser.perform(function() {
      if (!routeAvailable) {
        console.log('⏭️  /patients/new route not available in this build. Skipping.');
        return;
      }
      ...
      browser.elements('css selector', 'h1, h2, main, .page-content', function(result) {
        browser.assert.ok(result.value && result.value.length > 0,
          'ONC 170.315(a)(5) - Page loaded with content elements');
      });
      ...
      console.log('✅ Base EHR 170.315(a)(5) (Demographics) route accessibility test passed');
    });

    browser
      .saveScreenshot('tests/screenshots/base-ehr_170.315.a.5.png')
      .end();
  }
};
