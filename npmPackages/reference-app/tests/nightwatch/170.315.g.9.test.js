// npmPackages/reference-app/tests/nightwatch/170.315.g.9.test.js

const testUtils = require('../../../../tests/nightwatch/honeycomb/enable_autopublish/shared-test-utils');

module.exports = {
  tags: ['base-ehr', 'onc-certification', '170.315.g.9'],

  'Base EHR - 170.315(g)(9) - Application Access - All Data Request': function (browser) {
    browser
    ...
    browser.execute(function() {
      ...
      if (!result.value || !result.value.isLoggedIn) {
        browser.executeAsync(function(done) {
          ...
          console.log('✅ Test user logged in for ONC 170.315(g)(9)');
        });
      }
    });
    ...
    browser.perform(function() {
      ...
      browser.elements('css selector', 'h1, h2, main, .page-content', function(result) {
        browser.assert.ok(result.value && result.value.length > 0,
          'ONC 170.315(g)(9) - Page loaded with content elements');
      });
      ...
      console.log('✅ Base EHR 170.315(g)(9) (Application Access - All Data Request) route accessibility test passed');
    });

    browser
      .saveScreenshot('tests/screenshots/base-ehr_170.315.g.9.png')
      .end();
  }
};
