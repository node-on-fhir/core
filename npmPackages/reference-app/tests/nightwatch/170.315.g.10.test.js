// npmPackages/reference-app/tests/nightwatch/170.315.g.10.test.js

const testUtils = require('../../../../tests/nightwatch/honeycomb/enable_autopublish/shared-test-utils');

module.exports = {
  tags: ['base-ehr', 'onc-certification', '170.315.g.10'],

  'Base EHR - 170.315(g)(10) - Standardized API for Patient and Population Services': function (browser) {
    browser
    ...
    browser.execute(function() {
      ...
      if (!result.value || !result.value.isLoggedIn) {
        browser.executeAsync(function(done) {
          ...
          console.log('✅ Test user logged in for ONC 170.315(g)(10)');
        });
      }
    });

    testUtils.navigateUrl(browser, '/api-docs/');
    browser
      .waitForElementVisible('body', 3000)
      .pause(1000);

    // Skip gracefully if the route is not loaded in this build
    ...
    browser.perform(function() {
      if (!routeAvailable) {
        console.log('⏭️  /api-docs/ route not available in this build. Skipping.');
        return;
      }
      ...
      browser.elements('css selector', 'h1, h2, main, .page-content', function(result) {
        browser.assert.ok(result.value && result.value.length > 0,
          'ONC 170.315(g)(10) - Page loaded with content elements');
      });
      ...
      console.log('✅ Base EHR 170.315(g)(10) (Standardized API for Patient and Population Services) route accessibility test passed');
    });

    browser
      .saveScreenshot('tests/screenshots/base-ehr_170.315.g.10.png')
      .end();
  }
};
