// npmPackages/decision-support/tests/nightwatch/170.315.b.11.test.js

const testUtils = require('../../../../tests/nightwatch/honeycomb/enable_autopublish/shared-test-utils');

module.exports = {
  tags: ['base-ehr', 'onc-certification', '170.315.b.11'],

  'Base EHR - 170.315(b)(11) - Decision Support Interventions': function (browser) {
    browser
    ...
    browser.execute(function() {
      ...
      if (!result.value || !result.value.isLoggedIn) {
        browser.executeAsync(function(done) {
          ...
          console.log('✅ Test user logged in for ONC 170.315(b)(11)');
        });
      }
    });

    testUtils.navigateUrl(browser, '/decision-support');
    browser
    ...
    browser.perform(function() {
      if (!routeAvailable) {
        console.log('⏭️  /decision-support route not available — decision-support package not loaded in this build. Skipping.');
        return;
      }
      ...
      browser.elements('css selector', 'h1, h2, main, .page-content', function(result) {
        browser.assert.ok(result.value && result.value.length > 0,
          'ONC 170.315(b)(11) - Page loaded with content elements');
      });
      ...
      console.log('✅ Base EHR 170.315(b)(11) (Decision Support Interventions) route accessibility test passed');
    });

    browser
      .saveScreenshot('tests/screenshots/base-ehr_170.315.b.11.png')
      .end();
  }
};
