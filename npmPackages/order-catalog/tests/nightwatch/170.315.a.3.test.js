// npmPackages/order-catalog/tests/nightwatch/170.315.a.3.test.js

const testUtils = require('../../../../tests/nightwatch/honeycomb/enable_autopublish/shared-test-utils');

module.exports = {
  tags: ['base-ehr', 'onc-certification', '170.315.a.3'],

  'Base EHR - 170.315(a)(3) - CPOE - Diagnostic Imaging': function (browser) {
    browser
    ...
    testUtils.navigateUrl(browser, '/cpoe/diagnostic-imaging');
    browser
    ...
    browser.perform(function() {
      if (!routeAvailable) {
        console.log('⏭️  /cpoe/diagnostic-imaging route not available — order-catalog package not loaded in this build. Skipping.');
        return;
      }
      ...
      browser.elements('css selector', 'h1, h2, main, .page-content', function(result) {
        browser.assert.ok(result.value && result.value.length > 0,
          'ONC 170.315(a)(3) - Page loaded with content elements');
      });
      ...
      console.log('✅ Base EHR 170.315(a)(3) (CPOE - Diagnostic Imaging) route accessibility test passed');
    });

    browser
      .saveScreenshot('tests/screenshots/base-ehr_170.315.a.3.png')
      .end();
  }
};
