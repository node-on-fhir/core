...
module.exports = {
  ...
  'Order Catalog - 170.315(a)(2) - CPOE - Laboratory': function (browser) {
    browser
    ...
    browser.execute(function() {
      ...
      if (!result.value || !result.value.isLoggedIn) {
        browser.executeAsync(function(done) {
          ...
          console.log('✅ Test user logged in for ONC 170.315(a)(2)');
        });
      }
    });

    testUtils.navigateUrl(browser, '/cpoe/laboratory');
    browser
    ...
    browser.perform(function() {
      if (!routeAvailable) {
        console.log('⏭️  /cpoe/laboratory route not available — order-catalog package not loaded in this build. Skipping.');
        return;
      }

      // Check for multiple possible page indicators
      browser.elements('css selector', '#cpoeLabPage, [data-testid="cpoe-laboratory"], .cpoe-laboratory-page, h1, h2, main, .page-content', function(result) {
        if (result.value && result.value.length > 0) {
          browser.assert.ok(true, 'ONC 170.315(a)(2) - Page loaded with content elements');
        } else {
          browser.assert.ok(false, 'ONC 170.315(a)(2) - No recognizable page elements found');
        }
      });
      ...
      // Log success
      browser.perform(function() {
        console.log('✅ ONC 170.315(a)(2) - Order Catalog (CPOE - Laboratory) route accessibility test passed');
      });
    });
