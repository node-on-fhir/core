// npmPackages/prescription-benefit/tests/nightwatch/170.315.b.4.test.js

const testUtils = require('../../../../tests/nightwatch/honeycomb/enable_autopublish/shared-test-utils');

module.exports = {
  tags: ['base-ehr', 'onc-certification', '170.315.b.4'],

  'Base EHR - 170.315(b)(4) - Real-time Prescription Benefit': function (browser) {
    browser
    ...
    browser.execute(function() {
      ...
      if (!result.value || !result.value.isLoggedIn) {
        browser.executeAsync(function(done) {
          ...
          console.log('✅ Test user logged in for ONC 170.315(b)(4)');
        });
      }
    });

    testUtils.navigateUrl(browser, '/prescription-benefit');
    browser
    ...
    browser.perform(function() {
      if (!routeAvailable) {
        console.log('⏭️  /prescription-benefit route not available — prescription-benefit package not loaded in this build. Skipping.');
        return;
      }
      ...
      browser.elements('css selector', 'h1, h2, main, .page-content', function(result) {
        browser.assert.ok(result.value && result.value.length > 0,
          'ONC 170.315(b)(4) - Page loaded with content elements');
      });
      ...
      console.log('✅ Base EHR 170.315(b)(4) (Real-time Prescription Benefit) route accessibility test passed');
    });

    browser
      .saveScreenshot('tests/screenshots/base-ehr_170.315.b.4.png')
      .end();
  }
};
