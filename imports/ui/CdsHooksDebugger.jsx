...
export default function CdsHooksDebugger(props){
  console.log('CdsHooksDebugger', props)
  ...
  const { theme, toggleTheme } = useTheme();
    ...
    let [cdsHooksDiscovery, setCdsHooksDiscovery] = useState([]);
    let [selectedCardId, setSelectedCardId] = useState("");
    let [selectedHook, setSelectedHook] = useState(null);
    let [returnedCards, setReturnedCards] = useState([]);
    ...
    useEffect(function(){
      ...
      async function fetchData(){
        ...
        // await fetch(cdsHooksServicesUrl, {
        //   method: 'GET',
        //   headers: {
    ...
    let cdsHookServiceQueryElements = [];
    if(get(window, 'location.search')){
      cdsHookServiceQueryElements.push(<CardHeader key={Random.id()} title="Query CDS Hooks Service" subheader="Parameters that are pased in via URL during the application redirect process." />)
      cdsHookServiceQueryElements.push(<Card key={Random.id()} >
        <CardContent>
      ...
      cdsHookServiceQueryElements.push(<br key={Random.id()} />);
      cdsHookServiceQueryElements.push(<Card key={Random.id()}> 
        <CardContent>
          <AceEditor
            mode="text"
      ...
      let cdsHooksServiceUrl = "";
      if(get(selectedHook, 'hook')){
        cdsHooksServiceUrl = get(Meteor, 'settings.public.smartOnFhir[0].cdsHooksServices', "http://localhost:3000") + "/" + get(selectedHook, 'hook');
      }
      

      cdsHookServiceQueryElements.push(<CardHeader key={Random.id()} title="CDS Service Query" subheader="Parameters that are pased in via URL during the application redirect process." />)
      cdsHookServiceQueryElements.push(<br key={Random.id()} />);
      cdsHookServiceQueryElements.push(<Card key={Random.id()}>
        <CardContent>
          <InputLabel>Service Url</InputLabel>
          <Input 
            id="cdsHooksServiceUrl" 
            variant="standard" 
            fullWidth
            value={ensureRealValue(cdsHooksServiceUrl)}
          />
        </CardContent>
      </Card>);
      cdsHookServiceQueryElements.push(<br key={Random.id()} />);
      cdsHookServiceQueryElements.push(<Button key={Random.id()} fullWidth variant="contained" color="primary" onClick={handlePostToHook.bind(this)}> 
        POST 
      </Button>);
      cdsHookServiceQueryElements.push(<br key={Random.id()} />);
    }
    ...
    if(cdsHooksDiscovery.length > 0){
      if(Array.isArray(cdsHooksDiscovery)){

        let prehookItems = [];
        cdsHooksDiscovery.forEach(function(service){

          if(get(service, 'prefetch')){
            Object.keys(service.prefetch).forEach(function(key){
              prehookItems.push(service.prefetch[key]);
              prehookItems.push(<br key={Random.id()} />);
            });
          }

          discoveredServices.push(<Alert key={Random.id()} severity="info" onClick={handleSetHook.bind(this, service)}>
            <AlertTitle>{get(service, 'id')}</AlertTitle>
            {get(service, 'hook')}<br/>
            {get(service, 'title')}<br/>
            {get(service, 'description')}<br/>
            { prehookItems }
          </Alert>)          
          discoveredServices.push(<br key={Random.id()} />);
        })
      }
    }


    let returnedCardElements = [];
    if(returnedCards.length > 0){
      if(Array.isArray(returnedCards)){
        returnedCards.forEach(function(card){

          let linkElements = []

          if(Array.isArray(get(card, 'links'))){
            card.links.forEach(function(link){
              linkElements.push(<a key={Random.id()} href={get(link, 'url')}>{get(link, 'label')}</a>);
              linkElements.push(<br key={Random.id()} />);
            });
          }


          returnedCardElements.push(<br key={Random.id()} />);
          returnedCardElements.push(<Card key={Random.id()}>
            <Alert severity="success">
              <AlertTitle>{get(card, 'summary')}</AlertTitle>
              {get(card, 'source.label')}<br/>
              {get(card, 'source.url')}<br/>
              <br/>
              { linkElements }
            </Alert>
          </Card>)
    ...
    let scopesElements = [];
    if(showScopes){
      scopesElements.push(<br key={Random.id()} />);
    ...
    return (
        <div id='SmartLauncher' style={{'height': window.innerHeight, 'overflow': "auto", padding: "20px"}} >
            ...
            <Grid container spacing={3} style={{width: '100%'}}>
              ...
              <Grid item xs={4} sm={4} md={4} lg={4} >
                <CardHeader title="CDS Hooks Discovery" subheader="These hooks are available on the server specified in Meteor.settings.public.smartOnFHIR[0].cdsHooksService" />
                {/* <Card>
                  <CardContent>
                    <TextField 
                      id="server_url" 
              ...
              <Grid item xs={4} sm={4} md={4} lg={4} >
                { cdsHookServiceQueryElements }
                { returnedCardElements }
              </Grid>
            </Grid>
            <br />
