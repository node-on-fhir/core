...
export function InsurancePlansPage(props){
    ...
    if(searchFilter && searchFilter.length > 0) {
      query = {
        $or: [
          ...
          {'alias': {$regex: searchFilter, $options: 'i'}},
          {'status': {$regex: searchFilter, $options: 'i'}},
          {'type.0.text': {$regex: searchFilter, $options: 'i'}},
          {'ownedBy.display': {$regex: searchFilter, $options: 'i'}}
        ]
      };
    }
  ...
  function handleAddInsurancePlan(){
    console.log('Add Insurance Plan button clicked');
      ...
      <Box mb={2}>
            ...
            <Box display="flex" gap={2} alignItems="center">
              ...
              <ToggleButtonGroup
                ...
                onChange={(event, newFormats) => {
                  setShowPeriod(newFormats.includes('period'));
                  setShowSystemId(newFormats.includes('systemId'));
                }}
                aria-label="display options"
                size="small"
              >
                <ToggleButton value="period" aria-label="show period columns">
                  <DateRangeIcon />
                </ToggleButton>
        ...
        <Box mt={2}>
          <TextField
            ...
            fullWidth
            placeholder="Search insurance plans by ID, name, alias, status, or owner..."
            value={searchFilter}
    ...
    >
      <CardContent sx={{ p: 0 }}>
        <InsurancePlansTable
          ...
          hidePeriodStart={!showPeriod}
          hidePeriodEnd={!showPeriod}
          order={sortOrder}
