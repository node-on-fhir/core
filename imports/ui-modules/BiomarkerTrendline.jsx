...
// Props:
//   series       { code, display, unit, isPercentageFraction, observations[] }
...
export function BiomarkerTrendlineChart(props) {
  ...
  // Y-axis range with padding (using normalized values).
  const values = series.observations.map(function(obs) {
    return getNormalizedValue(obs, series.unit, series.isPercentageFraction) || 0;
  });
      ...
      <ResponsiveLine
        ...
        tooltip={({ point }) => (
          <div style={{
            ...
            <div>{point.data.yFormatted} {series.unit}</div>
