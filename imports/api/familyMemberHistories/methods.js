...
Meteor.ServerMethods.define('familyMemberHistories.create', {
  description: 'Create a FHIR FamilyMemberHistory record for a patient',
  phi: true,
  aliases: ['createFamilyMemberHistory'],
  schemaObject: { type: 'object' }
}, async function(params, context){
  const familyMemberHistoryData = params;

  context.log.info('Creating family member history');
  ...
  try {
    ...
    context.log.info('Family member history created', { familyMemberHistoryId: newId });
    return newId;
  } catch (error) {
    context.log.error('Error creating family member history', { message: error.message });
    throw new Meteor.Error('create-failed', 'Failed to create family member history: ' + error.message);
  }
});

Meteor.ServerMethods.define('familyMemberHistories.update', {
  description: 'Update an existing FHIR FamilyMemberHistory record by MongoDB _id',
  phi: true,
  aliases: ['updateFamilyMemberHistory'],
  positionalParams: ['familyMemberHistoryId', 'familyMemberHistoryData'],
  schemaObject: {
    type: 'object',
    properties: {
      familyMemberHistoryId: { type: 'string' },
      familyMemberHistoryData: { type: 'object' }
    },
    required: ['familyMemberHistoryId', 'familyMemberHistoryData']
  }
}, async function(params, context){
  const { familyMemberHistoryId, familyMemberHistoryData } = params;

  context.log.info('Updating family member history', { familyMemberHistoryId: familyMemberHistoryId });
  ...
  try {
    ...
    context.log.info('Family member history updated', { familyMemberHistoryId: familyMemberHistoryId, updated: result });
    return result;
  } catch (error) {
    context.log.error('Error updating family member history', { message: error.message });
    throw new Meteor.Error('update-failed', 'Failed to update family member history: ' + error.message);
  }
});

Meteor.ServerMethods.define('familyMemberHistories.remove', {
  description: 'Remove a FHIR FamilyMemberHistory record by MongoDB _id',
  phi: true,
  aliases: ['removeFamilyMemberHistory'],
  positionalParams: ['familyMemberHistoryId'],
  schemaObject: {
    type: 'object',
    properties: { familyMemberHistoryId: { type: 'string' } },
    required: ['familyMemberHistoryId']
  }
}, async function(params, context){
  const familyMemberHistoryId = params.familyMemberHistoryId;

  context.log.info('Removing family member history', { familyMemberHistoryId: familyMemberHistoryId });

  try {
    const result = await FamilyMemberHistories.removeAsync({ _id: familyMemberHistoryId });
    context.log.info('Family member history removed', { familyMemberHistoryId: familyMemberHistoryId, removed: result });
    return result;
  } catch (error) {
    context.log.error('Error removing family member history', { message: error.message });
