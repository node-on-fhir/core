...
export function PersonsTable(props){
  ...
  if(logger){
    logger.trace('PersonsTable.persons', persons)
  }
  ...
  function renderRowAvatar(person, avatarStyle){
    //logger.log('renderRowAvatar', person, avatarStyle)
  ...
  function selectPersonRow(person, index){
    if(logger){
  ...
  function removeRecord(_id){
    if(logger){
      logger.debug('Remove person ', _id)
    }
    if(onRemoveRecord){
      onRemoveRecord(_id);
    }
  ...
  function renderCounts(cursors, index){
    ...
    if(Array.isArray(cursors)){
      ...
      // logger.log('PersonsTable.serializedCounts.array', serializedCounts, index, cursors[index])
    } else {
      serializedCounts = serializeCounts(cursors)
      // logger.log('PersonsTable.serializedCounts', serializedCounts)
    }
  ...
  if(personsToRender.length === 0){
    ...
    for (var i = 0; i < personsToRender.length; i++) {
      ...
      if(logger){
        logger.trace('PersonsTable.personsToRender[' + i + ']', personsToRender[i]);
      }
      ...
      tableRows.push(
        <TableRow key={i} className="personRow" hover={true} style={rowStyle} selected={selected} onClick={ selectPersonRow.bind(this, personsToRender[i] )} >
          ...
          { renderMaritalStatus(get(personsToRender[i], "maritalStatus")) }
