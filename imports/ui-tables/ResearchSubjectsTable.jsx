...
export function ResearchSubjectsTable(props){
  // logger.log('ResearchSubjectsTable', props)
  ...
  if(logger){
    logger.trace('ResearchSubjectsTable.researchSubjects', researchSubjects)
  }
    ...
    if (showActionButton === true) {
      return (
        <TableCell className='ActionButton' >
          <Button onClick={ handleActionButtonClick.bind('this', researchSubjectsToRender[i]._id)}>{ get(props, "actionButtonLabel", "") }</Button>
        </TableCell>
      );
    }
  }
  ...
  function renderCounts(cursors, index){
    ...
    if(Array.isArray(cursors)){
      ...
      // logger.log('ResearchSubjectsTable.serializedCounts.array', serializedCounts, index, cursors[index])
    } else {
      serializedCounts = serializeCounts(cursors)
      // logger.log('ResearchSubjectsTable.serializedCounts', serializedCounts)
    }
  ...
  if(researchSubjects){
    if(researchSubjects.length > 0){            
      let count = 0;  
      researchSubjects.forEach(function(patient){
        if((count >= (page * rowsPerPageToRender)) && (count < (page + 1) * rowsPerPageToRender)){
          researchSubjectsToRender.push(flattenPatient(patient, dateFormat));
        }
        count++;
      });  
    }
  ...
  if(researchSubjectsToRender.length === 0){
    ...
    for (var i = 0; i < researchSubjectsToRender.length; i++) {
      ...
      if(logger){
        logger.trace('ResearchSubjectsTable.researchSubjectsToRender[' + i + ']', researchSubjectsToRender[i]);
      }

      let selected = false;
      if(researchSubjectsToRender[i].id === selectedPatientId){
        selected = true;
      }
      ...
      tableRows.push(
        <TableRow key={i} className="patientRow" hover={true} style={rowStyle} selected={selected} onClick={ selectPatientRow.bind(this, researchSubjectsToRender[i] )} >
          { renderActionIcons(researchSubjectsToRender[i]) }
          { renderRowAvatar(researchSubjectsToRender[i], styles.avatar) }
          { renderIdentifier(researchSubjectsToRender[i].identifier)}

          { renderName(get(researchSubjectsToRender[i], "name"), get(researchSubjectsToRender[i], "_id"))}
          { renderGender(get(researchSubjectsToRender[i], "gender"), get(researchSubjectsToRender[i], "_id"))}
          { renderBirthDate(get(researchSubjectsToRender[i], "birthDate"), get(researchSubjectsToRender[i], "_id"))}

          { renderAddress(get(researchSubjectsToRender[i], 'addressLine') ) }
          { renderCity(get(researchSubjectsToRender[i], 'city')) }
          { renderState(get(researchSubjectsToRender[i], 'state')) }
          { renderZipCode(get(researchSubjectsToRender[i], 'postalCode')) }
          { renderCountry(get(researchSubjectsToRender[i], 'country')) }

          { renderMaritalStatus(get(researchSubjectsToRender[i], "maritalStatus")) }
          { renderLanguage(get(researchSubjectsToRender[i], "preferredLanguage")) }
          { renderIsActive(get(researchSubjectsToRender[i], "active")) }

          { renderCounts(cursors, i) }
          { renderActionButton(researchSubjectsToRender[i], styles.avatar) }

          { renderSystemBarcode(researchSubjectsToRender[i]._id)}
          { renderBarcode(researchSubjectsToRender[i].id)}
        </TableRow>
      );
    }
  }
